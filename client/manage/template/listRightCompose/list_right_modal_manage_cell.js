Template.listRightModalManageCell.created = function() {
    ckPerms('ctrlperms');
}
/*
* 新闻模块cell
...
Template.listRightModalManageCell.events({
	...
	"click button.js-modal-up-bt" : function(e){// @上升@ 按钮
		...
		if(boxObj.prev().length > 0 ){//可以上升
			Meteor.call("upDownSecondRightModalData",updateid,sort,1,function(error,result){
				if(error){
					alert(UPDATE_ERROR);
					return false;
				}else{
	...
	"click button.js-modal-down-bt" : function(e){// @下降@ 按钮
		...
		if(boxObj.next().length > 0 ){//可以下降
			Meteor.call("upDownSecondRightModalData",updateid,sort,0,function(error,result){
				if(error){
					alert(UPDATE_ERROR);
					return false;
				}else{
